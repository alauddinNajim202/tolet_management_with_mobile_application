...
<div class="sticky">
    ...
    <div class="app-sidebar deep-dark-glass">
        ...
        <div class="sidebar-user-header floating-profile-card">
            <div class="user-avatar-initials">
                {{ substr(auth()->user()->name, 0, 1) }}{{ substr(strrchr(auth()->user()->name, " "), 1, 1) ?: '' }}
            </div>
            <div class="user-info">
                <h6 class="user-name text-white mb-0">{{ auth()->user()->name }}</h6>
                <p class="user-email text-white-50 mb-0">{{ auth()->user()->email }}</p>
            </div>
            <i class="fa fa-chevron-down ms-auto text-white-50" style="font-size: 10px;"></i>
        </div>
    </div>
</div>
<!--/APP-SIDEBAR-->
